...
ifeq ($BUILD_ENV, production)
	IVP_SOURCE_ENV=production
else
	ifeq ($BUILD_ENV, develop)
		IVP_SOURCE_ENV=develop
	endif
endif
...
help:
	...
	@echo
	@echo "make run_venv_code_check - run code style check using virtual environment"
...
run_venv_code_check:
	...
	.venv/bin/pylint gdelt20*
	.venv/bin/mypy gdelt20*
...
docker_build:
	...
	docker build . -t ivp/ivp-source-gdelt20:latest

docker_run:
	@echo ----- Run docker image -----
	docker run \
		-e IVP_SOURCE_ENV=${IVP_SOURCE_ENV} \
	  	ivp/ivp-source-gdelt20:latest
